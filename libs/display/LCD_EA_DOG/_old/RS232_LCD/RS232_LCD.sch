EESchema Schematic File Version 2
LIBS:power
LIBS:device
LIBS:transistors
LIBS:conn
LIBS:linear
LIBS:regul
LIBS:74xx
LIBS:cmos4000
LIBS:adc-dac
LIBS:memory
LIBS:xilinx
LIBS:microcontrollers
LIBS:dsp
LIBS:microchip
LIBS:analog_switches
LIBS:motorola
LIBS:texas
LIBS:intel
LIBS:audio
LIBS:interface
LIBS:digital-audio
LIBS:philips
LIBS:display
LIBS:cypress
LIBS:siliconi
LIBS:opto
LIBS:atmel
LIBS:contrib
LIBS:valves
LIBS:w_microcontrollers
LIBS:lib_villemejane
LIBS:RS232_LCD-cache
EELAYER 25 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title ""
Date "25 nov 2015"
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L LCD_DOG_163 LCD1
U 1 1 56509916
P 6200 1250
F 0 "LCD1" H 4750 950 60  0000 C CNN
F 1 "LCD_DOG_163" H 4800 850 60  0000 C CNN
F 2 "lib_villemejane:LCD_DOG" H 6200 1250 60  0001 C CNN
F 3 "" H 6200 1250 60  0000 C CNN
	1    6200 1250
	1    0    0    -1  
$EndComp
$Comp
L VCC #PWR01
U 1 1 5650991B
P 1250 1950
F 0 "#PWR01" H 1250 2050 30  0001 C CNN
F 1 "VCC" H 1250 2050 30  0000 C CNN
F 2 "" H 1250 1950 60  0000 C CNN
F 3 "" H 1250 1950 60  0000 C CNN
	1    1250 1950
	0    1    1    0   
$EndComp
$Comp
L VCC #PWR02
U 1 1 56509952
P 3600 1050
F 0 "#PWR02" H 3600 1150 30  0001 C CNN
F 1 "VCC" H 3600 1150 30  0000 C CNN
F 2 "" H 3600 1050 60  0000 C CNN
F 3 "" H 3600 1050 60  0000 C CNN
	1    3600 1050
	1    0    0    -1  
$EndComp
$Comp
L VCC #PWR03
U 1 1 565099F9
P 2200 1500
F 0 "#PWR03" H 2200 1600 30  0001 C CNN
F 1 "VCC" H 2200 1600 30  0000 C CNN
F 2 "" H 2200 1500 60  0000 C CNN
F 3 "" H 2200 1500 60  0000 C CNN
	1    2200 1500
	1    0    0    -1  
$EndComp
Wire Wire Line
	3600 1050 3600 1150
Wire Wire Line
	3700 1050 3700 1150
Wire Wire Line
	2800 1550 2550 1550
Wire Wire Line
	2800 1850 2650 1850
Wire Wire Line
	2650 1850 2650 1550
Connection ~ 2650 1550
Wire Wire Line
	2800 1950 2700 1950
Wire Wire Line
	2700 1650 2700 2050
Wire Wire Line
	2700 1650 2800 1650
Connection ~ 2700 1950
Wire Wire Line
	2250 1550 2200 1550
Wire Wire Line
	2200 1550 2200 1500
Wire Wire Line
	1000 1450 1250 1450
Text GLabel 5400 3550 2    60   Input ~ 0
RS
Text GLabel 5400 4150 2    60   Input ~ 0
SCK
Text GLabel 3400 4050 0    60   Input ~ 0
MOSI
Wire Wire Line
	1000 1050 1250 1050
Wire Wire Line
	1000 1150 1250 1150
Wire Wire Line
	1000 1250 1250 1250
Wire Wire Line
	1000 1350 1250 1350
$Comp
L VCC #PWR04
U 1 1 56509CA5
P 5200 1050
F 0 "#PWR04" H 5200 1150 30  0001 C CNN
F 1 "VCC" H 5200 1150 30  0000 C CNN
F 2 "" H 5200 1050 60  0000 C CNN
F 3 "" H 5200 1050 60  0000 C CNN
	1    5200 1050
	1    0    0    -1  
$EndComp
Wire Wire Line
	3950 1150 3950 1100
Wire Wire Line
	3950 1100 3700 1100
Connection ~ 3700 1100
Wire Wire Line
	4200 1150 4200 1100
Wire Wire Line
	4200 1100 5200 1100
Wire Wire Line
	4300 1100 4300 1150
Wire Wire Line
	5200 1050 5200 1150
Connection ~ 4300 1100
Connection ~ 5200 1100
Text GLabel 5400 2550 3    60   Input ~ 0
CS
Text GLabel 5300 2550 3    60   Input ~ 0
RS
Text GLabel 4100 2550 3    60   Input ~ 0
MOSI
Text GLabel 4200 2550 3    60   Input ~ 0
SCK
$Comp
L VCC #PWR05
U 1 1 56509DF7
P 4600 2650
F 0 "#PWR05" H 4600 2750 30  0001 C CNN
F 1 "VCC" H 4600 2750 30  0000 C CNN
F 2 "" H 4600 2650 60  0000 C CNN
F 3 "" H 4600 2650 60  0000 C CNN
	1    4600 2650
	-1   0    0    1   
$EndComp
Wire Wire Line
	4100 2550 4100 2500
Wire Wire Line
	4200 2550 4200 2500
Wire Wire Line
	4600 2650 4600 2500
Wire Wire Line
	4300 2500 4300 2550
Wire Wire Line
	4300 2550 5000 2550
Connection ~ 4600 2550
Wire Wire Line
	4400 2500 4400 2550
Connection ~ 4400 2550
Wire Wire Line
	4500 2500 4500 2550
Connection ~ 4500 2550
Wire Wire Line
	4700 2550 4700 2500
Wire Wire Line
	4800 2550 4800 2500
Connection ~ 4700 2550
Wire Wire Line
	5000 2550 5000 2500
Connection ~ 4800 2550
Wire Wire Line
	5100 2500 5100 2650
Wire Wire Line
	5300 2500 5300 2550
Wire Wire Line
	5400 2500 5400 2550
Text GLabel 2650 2400 0    60   Input ~ 0
SCK
Wire Wire Line
	2650 2400 2700 2400
Wire Wire Line
	2700 2400 2700 2450
Wire Wire Line
	2700 2750 2700 2850
$Comp
L PIC16F1509 U1
U 1 1 58B5441E
P 4400 3450
F 0 "U1" H 4350 3900 60  0000 C CNN
F 1 "PIC16F1509" H 4350 3800 60  0000 C CNN
F 2 "Housings_DIP:DIP-20_W7.62mm_LongPads" H 4100 3450 60  0001 C CNN
F 3 "" H 4100 3450 60  0000 C CNN
	1    4400 3450
	1    0    0    -1  
$EndComp
Text GLabel 1250 1050 2    60   Input ~ 0
MCLR
$Comp
L VCC #PWR06
U 1 1 58B54557
P 3050 3250
F 0 "#PWR06" H 3050 3350 30  0001 C CNN
F 1 "VCC" H 3050 3350 30  0000 C CNN
F 2 "" H 3050 3250 60  0000 C CNN
F 3 "" H 3050 3250 60  0000 C CNN
	1    3050 3250
	0    -1   -1   0   
$EndComp
Text GLabel 5400 3350 2    60   Input ~ 0
PGD
Text GLabel 5400 3450 2    60   Input ~ 0
PGC
Text GLabel 3050 3550 0    60   Input ~ 0
MCLR
$Comp
L R R1
U 1 1 58B548F2
P 2400 1550
F 0 "R1" V 2480 1550 50  0000 C CNN
F 1 "47" V 2400 1550 50  0000 C CNN
F 2 "Discret:R5" V 2330 1550 50  0001 C CNN
F 3 "" H 2400 1550 50  0000 C CNN
	1    2400 1550
	0    1    1    0   
$EndComp
$Comp
L R R2
U 1 1 58B549DC
P 2700 2600
F 0 "R2" V 2780 2600 50  0000 C CNN
F 1 "270" V 2700 2600 50  0000 C CNN
F 2 "Discret:R5" V 2630 2600 50  0001 C CNN
F 3 "" H 2700 2600 50  0000 C CNN
	1    2700 2600
	1    0    0    -1  
$EndComp
$Comp
L R R3
U 1 1 58B54AEB
P 3200 3400
F 0 "R3" V 3280 3400 50  0000 C CNN
F 1 "1.2k" V 3200 3400 50  0000 C CNN
F 2 "Discret:R5" V 3130 3400 50  0001 C CNN
F 3 "" H 3200 3400 50  0000 C CNN
	1    3200 3400
	1    0    0    -1  
$EndComp
Wire Wire Line
	3050 3550 3500 3550
Connection ~ 3200 3550
Wire Wire Line
	3050 3250 3500 3250
Connection ~ 3200 3250
Text GLabel 1250 1350 2    60   Input ~ 0
PGD
Text GLabel 1250 1450 2    60   Input ~ 0
PGC
Text Notes 600  1400 1    60   ~ 0
ICSP
Text Notes 600  2100 1    60   ~ 0
FTDI
Text GLabel 1250 1850 2    60   Input ~ 0
CTS
Text GLabel 1250 2250 2    60   Input ~ 0
RTS
Text GLabel 1250 2050 2    60   Input ~ 0
TX
Text GLabel 1250 2150 2    60   Input ~ 0
RX
Text GLabel 3400 4150 0    60   Input ~ 0
RX
Text GLabel 5400 4050 2    60   Input ~ 0
TX
Wire Wire Line
	1000 2250 1250 2250
Wire Wire Line
	1000 2150 1250 2150
Wire Wire Line
	1000 2050 1250 2050
Wire Wire Line
	1000 1950 1250 1950
Wire Wire Line
	1000 1850 1250 1850
Wire Wire Line
	1000 1750 1250 1750
Wire Wire Line
	5300 3350 5400 3350
Wire Wire Line
	5300 3450 5400 3450
$Comp
L R R4
U 1 1 58B5521A
P 3250 3650
F 0 "R4" V 3330 3650 50  0000 C CNN
F 1 "100" V 3250 3650 50  0000 C CNN
F 2 "Discret:R5" V 3180 3650 50  0001 C CNN
F 3 "" H 3250 3650 50  0000 C CNN
	1    3250 3650
	0    1    1    0   
$EndComp
$Comp
L LED D1
U 1 1 58B55282
P 2900 3650
F 0 "D1" H 2900 3750 50  0000 C CNN
F 1 "LED" H 2900 3550 50  0000 C CNN
F 2 "LEDs:LED-5MM" H 2900 3650 50  0001 C CNN
F 3 "" H 2900 3650 50  0000 C CNN
	1    2900 3650
	1    0    0    -1  
$EndComp
Wire Wire Line
	5300 3650 5400 3650
Wire Wire Line
	5300 4050 5400 4050
Wire Wire Line
	5300 4150 5400 4150
Wire Wire Line
	3400 4150 3500 4150
Wire Wire Line
	3400 4050 3500 4050
$Comp
L CONN_01X02 P1
U 1 1 58B5578F
P 800 750
F 0 "P1" H 800 900 50  0000 C CNN
F 1 "CONN_01X02" V 900 750 50  0000 C CNN
F 2 "Pin_Headers:Pin_Header_Straight_1x02" H 800 750 50  0001 C CNN
F 3 "" H 800 750 50  0000 C CNN
	1    800  750 
	-1   0    0    1   
$EndComp
$Comp
L CONN_01X06 P2
U 1 1 58B558B1
P 800 1300
F 0 "P2" H 800 1650 50  0000 C CNN
F 1 "CONN_01X06" V 900 1300 50  0000 C CNN
F 2 "Pin_Headers:Pin_Header_Straight_1x06" H 800 1300 50  0001 C CNN
F 3 "" H 800 1300 50  0000 C CNN
	1    800  1300
	-1   0    0    1   
$EndComp
$Comp
L CONN_01X06 P3
U 1 1 58B55988
P 800 2000
F 0 "P3" H 800 2350 50  0000 C CNN
F 1 "CONN_01X06" V 900 2000 50  0000 C CNN
F 2 "Pin_Headers:Pin_Header_Straight_1x06" H 800 2000 50  0001 C CNN
F 3 "" H 800 2000 50  0000 C CNN
	1    800  2000
	-1   0    0    1   
$EndComp
Text Notes 600  900  1    60   ~ 0
POWER
$Comp
L VCC #PWR07
U 1 1 58B55C9B
P 1250 1150
F 0 "#PWR07" H 1250 1250 30  0001 C CNN
F 1 "VCC" H 1250 1250 30  0000 C CNN
F 2 "" H 1250 1150 60  0000 C CNN
F 3 "" H 1250 1150 60  0000 C CNN
	1    1250 1150
	0    1    1    0   
$EndComp
$Comp
L VCC #PWR08
U 1 1 58B55CC4
P 1250 700
F 0 "#PWR08" H 1250 800 30  0001 C CNN
F 1 "VCC" H 1250 800 30  0000 C CNN
F 2 "" H 1250 700 60  0000 C CNN
F 3 "" H 1250 700 60  0000 C CNN
	1    1250 700 
	0    1    1    0   
$EndComp
Wire Wire Line
	1000 700  1250 700 
Wire Wire Line
	1000 800  1250 800 
Text Notes 600  2850 1    60   ~ 0
SPI1
$Comp
L CONN_01X06 P5
U 1 1 58B5611D
P 800 3450
F 0 "P5" H 800 3800 50  0000 C CNN
F 1 "CONN_01X06" V 900 3450 50  0000 C CNN
F 2 "Pin_Headers:Pin_Header_Straight_1x06" H 800 3450 50  0001 C CNN
F 3 "" H 800 3450 50  0000 C CNN
	1    800  3450
	-1   0    0    1   
$EndComp
Text GLabel 1100 3300 2    60   Input ~ 0
CS2
Text GLabel 5400 3850 2    60   Input ~ 0
CS2
Text GLabel 5400 3750 2    60   Input ~ 0
CS1
Text GLabel 5400 3950 2    60   Input ~ 0
MISO
Text GLabel 1100 3600 2    60   Input ~ 0
MOSI
Text GLabel 1100 3400 2    60   Input ~ 0
SCK
Text GLabel 1100 3200 2    60   Input ~ 0
CS1
Text GLabel 1100 3500 2    60   Input ~ 0
MISO
Wire Wire Line
	1000 3200 1100 3200
Wire Wire Line
	1000 3300 1100 3300
Wire Wire Line
	1000 3400 1100 3400
Wire Wire Line
	1000 3500 1100 3500
Wire Wire Line
	1000 3600 1100 3600
Wire Wire Line
	1000 3700 1100 3700
$Comp
L CONN_01X06 P4
U 1 1 58B56E32
P 800 2750
F 0 "P4" H 800 3100 50  0000 C CNN
F 1 "CONN_01X06" V 900 2750 50  0000 C CNN
F 2 "Pin_Headers:Pin_Header_Straight_1x06" H 800 2750 50  0001 C CNN
F 3 "" H 800 2750 50  0000 C CNN
	1    800  2750
	-1   0    0    1   
$EndComp
Text GLabel 1100 2600 2    60   Input ~ 0
CS2
Text GLabel 1100 2500 2    60   Input ~ 0
CS1
Wire Wire Line
	1000 2500 1100 2500
Wire Wire Line
	1000 2600 1100 2600
Wire Wire Line
	1000 2700 1100 2700
Wire Wire Line
	1000 2800 1100 2800
Wire Wire Line
	1000 2900 1100 2900
Wire Wire Line
	1000 3000 1100 3000
Text Notes 600  3550 1    60   ~ 0
SPI2\n
$Comp
L C C1
U 1 1 58B56FE9
P 2100 800
F 0 "C1" H 2125 900 50  0000 L CNN
F 1 "100nF" H 2125 700 50  0000 L CNN
F 2 "Capacitors_ThroughHole:C_Rect_L7_W2.5_P5" H 2138 650 50  0001 C CNN
F 3 "" H 2100 800 50  0000 C CNN
	1    2100 800 
	1    0    0    -1  
$EndComp
$Comp
L VCC #PWR09
U 1 1 58B571A1
P 2100 650
F 0 "#PWR09" H 2100 750 30  0001 C CNN
F 1 "VCC" H 2100 750 30  0000 C CNN
F 2 "" H 2100 650 60  0000 C CNN
F 3 "" H 2100 650 60  0000 C CNN
	1    2100 650 
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR010
U 1 1 58B54F9C
P 3700 1050
F 0 "#PWR010" H 3700 800 50  0001 C CNN
F 1 "GND" H 3700 900 50  0000 C CNN
F 2 "" H 3700 1050 50  0000 C CNN
F 3 "" H 3700 1050 50  0000 C CNN
	1    3700 1050
	-1   0    0    1   
$EndComp
$Comp
L GND #PWR011
U 1 1 58B54FCA
P 1250 800
F 0 "#PWR011" H 1250 550 50  0001 C CNN
F 1 "GND" H 1250 650 50  0000 C CNN
F 2 "" H 1250 800 50  0000 C CNN
F 3 "" H 1250 800 50  0000 C CNN
	1    1250 800 
	0    -1   -1   0   
$EndComp
$Comp
L GND #PWR012
U 1 1 58B55044
P 1250 1250
F 0 "#PWR012" H 1250 1000 50  0001 C CNN
F 1 "GND" H 1250 1100 50  0000 C CNN
F 2 "" H 1250 1250 50  0000 C CNN
F 3 "" H 1250 1250 50  0000 C CNN
	1    1250 1250
	0    -1   -1   0   
$EndComp
$Comp
L GND #PWR013
U 1 1 58B55083
P 2100 950
F 0 "#PWR013" H 2100 700 50  0001 C CNN
F 1 "GND" H 2100 800 50  0000 C CNN
F 2 "" H 2100 950 50  0000 C CNN
F 3 "" H 2100 950 50  0000 C CNN
	1    2100 950 
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR014
U 1 1 58B550B5
P 1250 1750
F 0 "#PWR014" H 1250 1500 50  0001 C CNN
F 1 "GND" H 1250 1600 50  0000 C CNN
F 2 "" H 1250 1750 50  0000 C CNN
F 3 "" H 1250 1750 50  0000 C CNN
	1    1250 1750
	0    -1   -1   0   
$EndComp
$Comp
L GND #PWR015
U 1 1 58B55101
P 2700 2850
F 0 "#PWR015" H 2700 2600 50  0001 C CNN
F 1 "GND" H 2700 2700 50  0000 C CNN
F 2 "" H 2700 2850 50  0000 C CNN
F 3 "" H 2700 2850 50  0000 C CNN
	1    2700 2850
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR016
U 1 1 58B55133
P 2700 2050
F 0 "#PWR016" H 2700 1800 50  0001 C CNN
F 1 "GND" H 2700 1900 50  0000 C CNN
F 2 "" H 2700 2050 50  0000 C CNN
F 3 "" H 2700 2050 50  0000 C CNN
	1    2700 2050
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR017
U 1 1 58B5517F
P 1100 3000
F 0 "#PWR017" H 1100 2750 50  0001 C CNN
F 1 "GND" H 1100 2850 50  0000 C CNN
F 2 "" H 1100 3000 50  0000 C CNN
F 3 "" H 1100 3000 50  0000 C CNN
	1    1100 3000
	0    -1   -1   0   
$EndComp
$Comp
L GND #PWR018
U 1 1 58B551B1
P 1100 3700
F 0 "#PWR018" H 1100 3450 50  0001 C CNN
F 1 "GND" H 1100 3550 50  0000 C CNN
F 2 "" H 1100 3700 50  0000 C CNN
F 3 "" H 1100 3700 50  0000 C CNN
	1    1100 3700
	0    -1   -1   0   
$EndComp
$Comp
L GND #PWR019
U 1 1 58B55214
P 5400 3250
F 0 "#PWR019" H 5400 3000 50  0001 C CNN
F 1 "GND" H 5400 3100 50  0000 C CNN
F 2 "" H 5400 3250 50  0000 C CNN
F 3 "" H 5400 3250 50  0000 C CNN
	1    5400 3250
	0    -1   -1   0   
$EndComp
Wire Wire Line
	5300 3250 5400 3250
$Comp
L GND #PWR021
U 1 1 58B553CB
P 5100 2650
F 0 "#PWR021" H 5100 2400 50  0001 C CNN
F 1 "GND" H 5100 2500 50  0000 C CNN
F 2 "" H 5100 2650 50  0000 C CNN
F 3 "" H 5100 2650 50  0000 C CNN
	1    5100 2650
	1    0    0    -1  
$EndComp
Wire Wire Line
	5300 3750 5400 3750
Wire Wire Line
	5300 3850 5400 3850
Wire Wire Line
	5300 3950 5400 3950
Text GLabel 1100 2700 2    60   Input ~ 0
SCK
Text GLabel 1100 2800 2    60   Input ~ 0
MISO
Text GLabel 1100 2900 2    60   Input ~ 0
MOSI
$Comp
L GND #PWR?
U 1 1 58B55F12
P 2700 3650
F 0 "#PWR?" H 2700 3400 50  0001 C CNN
F 1 "GND" H 2700 3500 50  0000 C CNN
F 2 "" H 2700 3650 50  0000 C CNN
F 3 "" H 2700 3650 50  0000 C CNN
	1    2700 3650
	0    1    1    0   
$EndComp
Wire Wire Line
	3400 3650 3500 3650
Text GLabel 5400 3650 2    60   Input ~ 0
CS
Wire Wire Line
	5300 3550 5400 3550
$EndSCHEMATC
